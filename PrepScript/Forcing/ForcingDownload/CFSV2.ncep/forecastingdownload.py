...
start_year = 2026
end_year = 2026
base_dir = "/hydata01/wumej22/SoftwareCopyright/CPS/PrepScript/Forcing/ForcingDownload/CFSV2.ncep"
init_time = "012900"  # 起报时间改为4月1日00时
# 输出目录
...
for year in range(start_year, end_year + 1):
    timelist.append(datetime(year, 4, 1))
...
for init_date in timelist:
    ...
    # 计算预报结束日期（11月1日）
    end_date = datetime(int(year), 2, 1)
    # 计算预报总小时数（从4月1日00时到11月1日00时）
    total_hours = int((end_date - datetime(int(year), 1, 29)).total_seconds() / 3600)
    ...
    for fhr in range(0, total_hours + 1, 6):
        # 计算目标日期时间
        target_time = datetime(int(year), 1, 29) + timedelta(hours=fhr)
